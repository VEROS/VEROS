...
Definition set_current_thread s t := mkS (Scheduler_Implementation.set_current_thread s.(sched_imp) t).
...
set (current := get_current_thread s). 
assert (s' : Scheduler). destruct (get_need_reschedule s) as [ | ]; [ |exact s].
  assert (s'' : Scheduler). destruct (schedule s) as [next|]; [ |exact s].
    destruct (Thread_Obj.eq_Obj current next) as [ | ]; [exact s| ].
      ...
      set (s_tmp3 := set_current_thread s_tmp2 current).
...
|Some next => set_current_thread (clear_need_reschedule s) next (*HAL_THREAD_LOAD_CONTEXT*)
|None => s (*shouldn't be*)
end.
...
set (s' := timeslice_restore (set_current_thread (clear_need_reschedule s) t) (timeslice_reset t)). 
